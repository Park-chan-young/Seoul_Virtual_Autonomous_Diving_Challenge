...
class latticePlanner:
    def __init__(self):
        ...
        rospy.Subscriber("Laser_to_Map", PoseArray, self.cluster_callback)        
        ...
        self.is_odom = False
        self.previous_obstacles = []  # 이전에 감지된 장애물의 위치를 저장하는 리스트
        self.lane_weight = [3, 3]

        rate = rospy.Rate(30) # 30hz
        while not rospy.is_shutdown():
            if self.is_path and self.is_status and self.is_cluster:
                if self.checkObject(self.local_path, self.cluster_data, self.previous_obstacles):
                    lattice_path = self.latticePlanner(self.local_path, self.status_msg, self.odom_msg)
                    lattice_path_index = self.collision_check(self.previous_obstacles, lattice_path, self.lane_weight)
    ...
    def checkObject(self, ref_path, cluster_data, previous_obstacles):

        is_crash = False
        obstacles = []  # 현재 감지된 장애물의 위치를 저장하는 리스트
        for obstacle in cluster_data.poses:
            obstacles.append((obstacle.position.x, obstacle.position.y))
        
        for obstacle in obstacles:
            for path in ref_path.poses:  
                dis = sqrt(pow(path.pose.position.x - obstacle[0], 2) + pow(path.pose.position.y - obstacle[1], 2))                
                if dis < 2: # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단. 4
                    is_crash = True
                    previous_obstacles.append((obstacle[0], obstacle[1]))
                    break
        if len(previous_obstacles):             
    ...
    def collision_check(self, previous_obstacles, out_path, lane_weight):
        ...
        for obstacle in previous_obstacles:                   
            for path_num in range(len(out_path)) :                    
                for path_pos in out_path[path_num].poses :
                    ...
                    dis = sqrt(pow(path_pos.pose.position.x - obstacle[0], 2) + pow(path_pos.pose.position.y - obstacle[1], 2))
                    lane_weight[path_num] = lane_weight[path_num] + 50*dis
    ...
    #     is_crash = False
    #     for obstacle in cluster_data.poses:
    #         dis = sqrt(pow(obstacle.position.x, 2) + pow(obstacle.position.y, 2))                
    #         if dis < 2.35: # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단. 4
    #             is_crash = True
    #             print(obstacle.position.x)
    #             print(" ")
    ...
    def latticePlanner(self,ref_path, vehicle_status, odom_status):
        out_path = []
        vehicle_pose_x = odom_status.pose.pose.position.x
        vehicle_pose_y = odom_status.pose.pose.position.y
